...
下面是一份已经完成“第一步：按发言顺序整理”的材料。请基于它生成“第二步：总结”。

要求：
1. 只基于提供的材料总结，不要引入材料中没有的信息。
2. 输出必须包含且仅包含以下三个部分：
   - **议题与观点**
   - **关键论据与证据**
   - **分歧与开放性问题**
3. “议题与观点”按议题归类，清楚写出不同说话人的具体主张或判断，不要把不同立场混成共识。
4. “关键论据与证据”收录材料中的数据、事实、案例、经历、研究、引用、类比或其他支撑性内容，并尽量标明对应说话人。
5. “分歧与开放性问题”收录明确分歧、保留意见、不确定性、尚未解决的问题，以及需要进一步验证之处。
6. 要覆盖完整，但表达可以高效、紧凑。
...
_STEP2_CHUNK_PROMPT = """\
下面是“第一步：按发言顺序整理”材料的第 {part} / {total} 部分。请只提取这一部分中对第二步总结有用的信息。{context_block}
...
要求：
1. 只提取这一部分材料里的信息，不要补写其他部分。
2. 在“议题与观点”中，按议题列出本部分出现的具体主张、判断和立场，并尽量标明说话人。
3. 在“关键论据与证据”中，列出本部分出现的数据、事实、案例、经历、研究、引用、类比等支撑内容。
4. 在“分歧与开放性问题”中，列出本部分出现的分歧、保留意见、不确定性和未决问题。
5. 如果上文片段仅用于衔接参考，不要重复输出它。

第一步材料片段：
{notes}"""

_STEP2_FINAL_PROMPT = """\
下面是基于“第一步：按发言顺序整理”材料分段提取出来的第二步笔记。请把它们合并成一份完整的“第二步：总结”。
...
1. 输出必须包含且仅包含以下三个部分：
   ...
   - **关键论据与证据**
   - **分歧与开放性问题**
2. 按议题合并同类内容，但不要丢失任何具体信息。
3. 不要把不同说话人的不同立场压平为单一结论。
4. 可以去除完全重复的信息，但不要因为“看起来相近”就删除细节。
5. 用简体中文输出，表达紧凑，但信息覆盖完整。
