...
def _wow_city_table(df, target_date, last_week_date):
    ...
    if df_this.empty:
        return f'<p style="color:#c00;">警告：未找到 {target_date} 的 Guming 数据。</p>'
    if df_last.empty:
        return f'<p style="color:#c00;">上周 ({last_week_date}) 数据缺失，Guming WoW 对比无法计算。</p>'

    common = set(df_this['Store Name'].unique()) & set(df_last['Store Name'].unique())
    if not common:
        return '<p style="color:#c00;">未找到两个日期均存在的 Guming 门店。</p>'
...
def analyze_stores(file_path, input_date_str=None):
    if not os.path.exists(file_path):
        return f'<p>错误：未找到 Guming 结果文件 "{file_path}"。</p>'
    try:
        df = pd.read_excel(file_path)
    except Exception as e:
        return f'<p>读取 Guming Excel 文件时出错：{e}</p>'
...
def cups_per_store_chart(file_path):
    """
    Returns a base64-encoded PNG of average cups-per-store over time.
    Only includes days where total store count > 10 to permit early stats.
    """
    import matplotlib
    matplotlib.use('Agg')
    ...
    # Dynamic filter: permit runs with at least 10 stores to plot trends
    daily = daily[daily['store_count'] > 10].copy()
    ...
    ax.set_title('古茗每日店均杯数趋势', fontsize=13, pad=10)
...
if __name__ == "__main__":
    FILE_PATH = "guming_city_stores.xlsx"
